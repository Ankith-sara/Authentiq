...
interface Group { id: string; name: string; description: string | null; created_by: string; is_public: boolean; created_at: string; member_count?: number; submission_count?: number; }
...
const Groups = () => {
  ...
  const joinGroup = async (groupId: string, silent = false) => {
    try {
  ...
  const rowStyle: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid #2563eb", gap: 16 };
  const btnStyle = (primary?: boolean): React.CSSProperties => ({
    ...
    borderColor: primary ? "#ffffff" : "rgba(255,255,255,0.1)",
    ...
    <div style={{ minHeight: "100vh", background: "#000000" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "100px 24px 80px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32, gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "#ffffff" }}>Create or join a group to compare originality scores on a live leaderboard.</p>
          </div>
        ...
        {/* Tabs */}
        <div style={{ display: "flex", gap: 0, marginBottom: 24, borderBottom: "1px solid #2563eb" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}><Loader2 size={24} color="#2563eb" style={{ animation: "spin 1s linear infinite", margin: "0 auto" }} /></div>
        ) : displayed.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 24px", borderRadius: 12, border: "1px solid #2563eb", background: "rgba(255,255,255,0.01)" }}>
            {tab === "mine" ? <Users size={32} color="#2563eb" style={{ margin: "0 auto 16px" }} /> : <Shield size={32} color="#2563eb" style={{ margin: "0 auto 16px" }} />}
            <h3 style={{ fontSize: 16, fontWeight: 600, color: "#ffffff", marginBottom: 8 }}>{tab === "mine" ? "No groups yet" : "No other groups"}</h3>
            <p style={{ fontSize: 13, color: "#2563eb", marginBottom: 20 }}>{tab === "mine" ? "Create a group or discover existing ones to join." : "You've joined all available groups."}</p>
          ...
          <div style={{ borderRadius: 12, border: "1px solid #2563eb", overflow: "hidden", background: "#000000" }}>
              ...
              return (
                <div key={group.id} style={{ ...rowStyle, background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flex: 1, minWidth: 0 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 9, background: "#2563eb", border: "1px solid #2563eb", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                        ...
                        {isMember && <span style={{ fontSize: 10, fontWeight: 600, color: "#ffffff", background: "#2563eb", border: "1px solid #2563eb", padding: "1px 7px", borderRadius: 99 }}>joined</span>}
                      </div>
                      <div style={{ display: "flex", gap: 12 }}>
                        <span style={{ fontSize: 12, color: "#2563eb" }}><span style={{ color: "#ffffff" }}>{group.member_count}</span> members</span>
                        <span style={{ fontSize: 12, color: "#2563eb" }}><span style={{ color: "#ffffff" }}>{group.submission_count}</span> checks</span>
                      </div>
                    </div>
                  </div>
        ...
        {myGroups.length > 0 && (
          <div style={{ marginTop: 32, padding: "20px 24px", borderRadius: 10, border: "1px solid #2563eb", background: "rgba(255,255,255,0.01)" }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>How Groups Work</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {["Go to Checker and select a group before checking", "Your originality score is saved to the group", "View the leaderboard to compare with others"].map((step, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#2563eb", width: 18, height: 18, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>{i + 1}</span>
                  <p style={{ fontSize: 12, color: "#ffffff", lineHeight: 1.5 }}>{step}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {createOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
          <div style={{ background: "#000000", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "28px", width: "100%", maxWidth: 420 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g., CS101 — Section A"
                style={{ width: "100%", padding: "10px 14px", borderRadius: 8, fontSize: 14, background: "#2563eb", border: "1px solid #2563eb", color: "#ffffff", outline: "none", boxSizing: "border-box" }} />
            </div>
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="What's this group for?" rows={3}
                style={{ width: "100%", padding: "10px 14px", borderRadius: 8, fontSize: 14, background: "#2563eb", border: "1px solid #2563eb", color: "#ffffff", outline: "none", boxSizing: "border-box", resize: "vertical" }} />
            </div>
            <button onClick={createGroup} disabled={creating} style={{ width: "100%", padding: "11px", borderRadius: 8, fontSize: 14, fontWeight: 600, background: creating ? "rgba(245,245,245,0.5)" : "#ffffff", color: "#000000", border: "none", cursor: creating ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
